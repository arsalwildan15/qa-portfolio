/**
 * ========================================================================
 * TC-LOGIN-24 — XSS img/onerror payload in password does not execute
 * Module  : Authentication — Login
 * Type    : [S] Security
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-24-Verify-XSS-In-Password-Is-Sanitized.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that xSS img/onerror payload in password does not execute.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-24-Verify-XSS-In-Password-Is-Sanitized.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 4 — SECURITY: an img/onerror XSS payload in the password must not execute.
test.describe('Login — Authentication', () => {
  ...
  // [S] Security: an <img onerror=...> XSS payload in the password must never execute either.
  test('TC-LOGIN-24 — XSS img/onerror payload in password does not execute', async ({ page }) => {
    ...
    console.log('[STEP 3] Fill XSS payload in password: <img src=x onerror=alert(1)>');
    await loginPage.fillPassword('<img src=x onerror=alert(1)>');
    checks.push({ label: 'Fill XSS payload in password', ok: true, detail: 'filled' });
    ...
    printTcSummary('TC-LOGIN-24', 'XSS img/onerror payload in password does not execute', checks);

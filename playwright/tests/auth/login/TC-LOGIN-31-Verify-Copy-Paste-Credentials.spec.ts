/**
 * ========================================================================
 * TC-LOGIN-31 — Typing credentials key-by-key logs in successfully
 * Module  : Authentication — Login
 * Type    : [!] Edge
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-31-Verify-Copy-Paste-Credentials.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that typing credentials key-by-key logs in successfully.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-31-Verify-Copy-Paste-Credentials.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 5 — EDGE: typing key-by-key should behave the same as a direct fill().
test.describe('Login — Authentication', () => {
  ...
  // [!] Edge: filling fields key-by-key (like a real user typing/pasting) should behave the same as fill().
  test('TC-LOGIN-31 — Typing credentials key-by-key logs in successfully', async ({ page }) => {
    ...
    console.log(`[STEP 2] Type username "${VALID_USERNAME}" key-by-key...`);
    await page.getByPlaceholder('Username').pressSequentially(VALID_USERNAME);
    checks.push({ label: 'Fill username key-by-key', ok: true, detail: 'filled' });

    console.log('[STEP 3] Type password key-by-key...');
    await page.getByPlaceholder('Password').pressSequentially(VALID_PASSWORD);
    checks.push({ label: 'Fill password key-by-key', ok: true, detail: 'filled' });
    ...
    printTcSummary('TC-LOGIN-31', 'Typing credentials key-by-key logs in successfully', checks);

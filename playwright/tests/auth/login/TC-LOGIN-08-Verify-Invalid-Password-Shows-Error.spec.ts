/**
 * ========================================================================
 * TC-LOGIN-08 — Invalid password is rejected
 * Module  : Authentication — Login
 * Type    : [-] Negative
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-08-Verify-Invalid-Password-Shows-Error.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that invalid password is rejected.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-08-Verify-Invalid-Password-Shows-Error.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 2 — NEGATIVE: the right username with the wrong password must be rejected.
test.describe('Login — Authentication', () => {
  ...
  // [-] Negative: the right username with the wrong password should be rejected.
  test('TC-LOGIN-08 — Invalid password is rejected', async ({ page }) => {
    ...
    console.log(`[STEP 3] Fill password "wrongpassword"...`);
    ...
    const response = await loginPage.submitAndInterceptExpectingRejection(VALID_USERNAME, 'wrongpassword');
    ...
    printTcSummary('TC-LOGIN-08', 'Invalid password is rejected', checks);

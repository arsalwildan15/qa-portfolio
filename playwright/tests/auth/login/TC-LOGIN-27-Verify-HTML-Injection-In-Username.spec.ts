/**
 * ========================================================================
 * TC-LOGIN-27 — HTML injection in username is not rendered as HTML
 * Module  : Authentication — Login
 * Type    : [S] Security
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-27-Verify-HTML-Injection-In-Username.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that hTML injection in username is not rendered as HTML.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-27-Verify-HTML-Injection-In-Username.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 4 — SECURITY: an HTML tag as username must render as plain text, never real HTML.
test.describe('Login — Authentication', () => {
  ...
  // [S] Security: an HTML tag as the username must be rendered as plain text, never as real HTML.
  test('TC-LOGIN-27 — HTML injection in username is not rendered as HTML', async ({ page }) => {
    ...
    console.log('[STEP 2] Fill HTML injection payload in username: <h1>Hacked</h1>');
    await loginPage.fillUsername('<h1>Hacked</h1>');
    checks.push({ label: 'Fill HTML payload in username', ok: true, detail: 'filled' });
    ...
    console.log('[STEP 5] Verify no real <h1>Hacked</h1> rendered in DOM...');
    const injectedHeadingCount = await page.locator('h1', { hasText: 'Hacked' }).count();
    checks.push({ label: 'No <h1>Hacked</h1> rendered', ok: injectedHeadingCount === 0, detail: `count=${injectedHeadingCount}` });

    printTcSummary('TC-LOGIN-27', 'HTML injection in username is not rendered as HTML', checks);

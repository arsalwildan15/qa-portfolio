/**
 * ========================================================================
 * TC-LOGIN-15 — Whitespace-only username cannot log in
 * Module  : Authentication — Login
 * Type    : [-] Negative
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-15-Verify-Whitespace-Only-Username-Shows-Validation.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that whitespace-only username cannot log in.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-15-Verify-Whitespace-Only-Username-Shows-Validation.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 2 — NEGATIVE: a username made only of spaces must never let the user in.
test.describe('Login — Authentication', () => {
  ...
  // [-] Negative: a username made only of spaces should not be treated as valid input.
  test('TC-LOGIN-15 — Whitespace-only username cannot log in', async ({ page }) => {
    ...
    console.log('[STEP 2] Fill username with spaces only "   "...');
    await loginPage.fillUsername('   ');
    checks.push({ label: 'Fill username "   " (spaces only)', ok: true, detail: 'filled' });
    ...
    console.log('[STEP 6] Verify still unable to log in...');
    const stillOnLogin = page.url().includes('/auth/login');
    checks.push({ label: 'URL stays at /auth/login', ok: stillOnLogin, detail: page.url() });

    printTcSummary('TC-LOGIN-15', 'Whitespace-only username cannot log in', checks);

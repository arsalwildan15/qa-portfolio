/**
 * ========================================================================
 * TC-LOGIN-09 — Both username and password invalid
 * Module  : Authentication — Login
 * Type    : [-] Negative
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-09-Verify-Both-Fields-Invalid-Shows-Error.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that both username and password invalid.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-09-Verify-Both-Fields-Invalid-Shows-Error.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 2 — NEGATIVE: both fields wrong at once must still fail cleanly.
test.describe('Login — Authentication', () => {
  ...
  // [-] Negative: both fields wrong at once should still fail cleanly.
  test('TC-LOGIN-09 — Both username and password invalid', async ({ page }) => {
    ...
    console.log(`[STEP 2] Fill username "wronguser"...`);
    await loginPage.fillUsername('wronguser');
    checks.push({ label: 'Fill username "wronguser"', ok: true, detail: 'filled' });

    console.log(`[STEP 3] Fill password "wrongpass"...`);
    ...
    // The shared public demo occasionally lets an already-authenticated
    // session bleed into this POST, making a deliberately-wrong login appear
    // to succeed — submitAndInterceptExpectingRejection() detects that and
    // retries once with a clean session before we assert the real behavior.
    const response = await loginPage.submitAndInterceptExpectingRejection('wronguser', 'wrongpass');
    ...
    checks.push({ label: 'No session cookie set', ok: setCookie === undefined, detail: setCookie === undefined ? 'confirmed' : 'FOUND COOKIE' });

    console.log('[STEP 5] Verify error message visible...');
    await loginPage.assertInvalidCredentialsError();
    checks.push({ label: 'Error message visible', ok: true, detail: 'Invalid credentials' });

    console.log('[STEP 6] Verify URL stays at /auth/login...');
    const stillOnLogin = page.url().includes('/auth/login');
    checks.push({ label: 'URL stays at /auth/login', ok: stillOnLogin, detail: page.url() });

    printTcSummary('TC-LOGIN-09', 'Both username and password invalid', checks);

/**
 * ========================================================================
 * TC-LOGIN-36 — Null byte in username does not crash the server or bypass auth
 * Module  : Authentication — Login
 * Type    : [!] Edge
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-36-Verify-Null-Byte-In-Username.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that null byte in username does not crash the server or bypass auth.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-36-Verify-Null-Byte-In-Username.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 5 — EDGE: a null byte is a classic input-sanitization attack vector.
test.describe('Login — Authentication', () => {
  ...
  // [!] Edge: a null byte embedded in the username is a classic input-sanitization attack vector.
  test('TC-LOGIN-36 — Null byte in username does not crash the server or bypass auth', async ({ page }) => {
    ...
    console.log('[STEP 2] Fill username with null byte "Admin\\u0000"...');
    await loginPage.fillUsername('Admin\u0000');
    checks.push({ label: 'Fill username with null byte', ok: true, detail: 'filled' });
    ...
    checks.push({ label: 'Location → /auth/login (not /dashboard)', ok: location.includes('/auth/login') && !location.includes('/dashboard'), detail: location });

    console.log('[STEP 5] Verify error message visible (no crash, no bypass)...');
    ...
    printTcSummary('TC-LOGIN-36', 'Null byte in username does not crash the server or bypass auth', checks);

/**
 * ========================================================================
 * TC-LOGIN-12 — Both fields empty shows two Required validations
 * Module  : Authentication — Login
 * Type    : [-] Negative
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-12-Verify-Both-Empty-Fields-Shows-Two-Validations.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that both fields empty shows two Required validations.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-12-Verify-Both-Empty-Fields-Shows-Two-Validations.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 2 — NEGATIVE: both fields empty must show two separate validation messages.
test.describe('Login — Authentication', () => {
  ...
  // [-] Negative: submitting with both fields empty should show two validation messages.
  test('TC-LOGIN-12 — Both fields empty shows two Required validations', async ({ page }) => {
    ...
    console.log('[STEP 2] Leave both fields empty...');
    checks.push({ label: 'Both fields left empty', ok: true, detail: 'empty' });
    ...
    console.log('[STEP 4] Verify no POST /auth/validate was sent...');
    checks.push({ label: 'No POST /auth/validate sent', ok: !submitted, detail: submitted ? 'REQUEST WAS SENT' : 'confirmed (FE validation)' });

    console.log('[STEP 5] Verify exactly 2 "Required" validations visible...');
    await loginPage.assertValidationErrorCount(2);
    checks.push({ label: 'Required validation count', ok: true, detail: '2' });

    printTcSummary('TC-LOGIN-12', 'Both fields empty shows two Required validations', checks);

/**
 * ========================================================================
 * TC-LOGIN-29 — Pressing Enter submits the login form
 * Module  : Authentication — Login
 * Type    : [!] Edge
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-29-Verify-Login-With-Enter-Key.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that pressing Enter submits the login form.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-29-Verify-Login-With-Enter-Key.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 5 — EDGE: pressing Enter is a very common real-world way to submit a form.
test.describe('Login — Authentication', () => {
  ...
  // [!] Edge: pressing Enter in the password field is a very common way real users submit forms.
  test('TC-LOGIN-29 — Pressing Enter submits the login form', async ({ page }) => {
    ...
    console.log('[STEP 4] Press Enter in the password field...');
    const [response] = await Promise.all([
      page.waitForResponse((res) => res.url().includes(VALIDATE_ENDPOINT) && res.request().method() === 'POST'),
      page.getByPlaceholder('Password').press('Enter'),
    ]);
    const status = response.status();
    const location = response.headers()['location'] ?? '';
    checks.push({ label: 'POST /auth/validate triggered by Enter', ok: true, detail: 'triggered' });
    ...
    printTcSummary('TC-LOGIN-29', 'Pressing Enter submits the login form', checks);

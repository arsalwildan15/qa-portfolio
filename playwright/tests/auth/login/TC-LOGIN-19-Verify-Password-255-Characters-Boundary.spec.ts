/**
 * ========================================================================
 * TC-LOGIN-19 — 255-character password does not crash the server
 * Module  : Authentication — Login
 * Type    : [B] Boundary
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-19-Verify-Password-255-Characters-Boundary.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that 255-character password does not crash the server.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-19-Verify-Password-255-Characters-Boundary.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 3 — BOUNDARY: a 255-char password must be handled like any wrong password.
test.describe('Login — Authentication', () => {
  ...
  // [B] Boundary: a 255-character password must be handled the same way as any wrong password.
  test('TC-LOGIN-19 — 255-character password does not crash the server', async ({ page }) => {
    ...
    console.log('[STEP 3] Fill password with 255 characters...');
    await loginPage.fillPassword(longPassword);
    checks.push({ label: 'Fill password (255 chars)', ok: true, detail: 'filled' });
    ...
    printTcSummary('TC-LOGIN-19', '255-character password does not crash the server', checks);

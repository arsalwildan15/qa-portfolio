/**
 * ========================================================================
 * TC-LOGIN-22 — SQL injection in password does not bypass login
 * Module  : Authentication — Login
 * Type    : [S] Security
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-22-Verify-SQL-Injection-In-Password.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that sQL injection in password does not bypass login.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-22-Verify-SQL-Injection-In-Password.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 4 — SECURITY: SQL injection payload in the password field must also fail safely.
test.describe('Login — Authentication', () => {
  ...
  // [S] Security: SQL injection payload in the password field must also fail safely.
  test('TC-LOGIN-22 — SQL injection in password does not bypass login', async ({ page }) => {
    ...
    console.log(`[STEP 3] Fill password "' OR '1'='1' --"...`);
    ...
    const response = await loginPage.submitAndInterceptExpectingRejection(VALID_USERNAME, "' OR '1'='1' --");
    ...
    printTcSummary('TC-LOGIN-22', 'SQL injection in password does not bypass login', checks);

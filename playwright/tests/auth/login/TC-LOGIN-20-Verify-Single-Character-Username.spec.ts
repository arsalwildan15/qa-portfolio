/**
 * ========================================================================
 * TC-LOGIN-20 — 1-character username "A" is rejected cleanly
 * Module  : Authentication — Login
 * Type    : [B] Boundary
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-20-Verify-Single-Character-Username.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that 1-character username "A" is rejected cleanly.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-20-Verify-Single-Character-Username.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 3 — BOUNDARY: the shortest possible non-empty username (1 char).
test.describe('Login — Authentication', () => {
  ...
  // [B] Boundary: the shortest possible non-empty username (1 character).
  test('TC-LOGIN-20 — 1-character username "A" is rejected cleanly', async ({ page }) => {
    ...
    console.log(`[STEP 2] Fill username "A"...`);
    await loginPage.fillUsername('A');
    checks.push({ label: 'Fill username "A"', ok: true, detail: 'filled' });
    ...
    const response = await loginPage.submitAndInterceptExpectingRejection('A', VALID_PASSWORD);
    ...
    printTcSummary('TC-LOGIN-20', '1-character username "A" is rejected cleanly', checks);

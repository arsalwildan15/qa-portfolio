/**
 * ========================================================================
 * TC-LOGIN-14 — Wrong-case password "Admin123" is rejected (case-sensitive)
 * Module  : Authentication — Login
 * Type    : [-] Negative
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-14-Verify-Case-Sensitive-Password.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that wrong-case password "Admin123" is rejected (case-sensitive).
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-14-Verify-Case-Sensitive-Password.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 2 — NEGATIVE: passwords are case-sensitive, wrong case must fail.
test.describe('Login — Authentication', () => {
  ...
  // [-] Negative: passwords are also case-sensitive, so wrong-case password must fail.
  test('TC-LOGIN-14 — Wrong-case password "Admin123" is rejected (case-sensitive)', async ({ page }) => {
    ...
    console.log(`[STEP 3] Fill password "Admin123 (wrong case)"...`);
    ...
    const response = await loginPage.submitAndInterceptExpectingRejection(VALID_USERNAME, 'Admin123');
    ...
    printTcSummary('TC-LOGIN-14', 'Wrong-case password "Admin123" is rejected (case-sensitive)', checks);

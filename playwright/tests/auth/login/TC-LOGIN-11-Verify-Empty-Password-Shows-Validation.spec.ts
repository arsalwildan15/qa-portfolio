/**
 * ========================================================================
 * TC-LOGIN-11 — Empty password shows Required validation
 * Module  : Authentication — Login
 * Type    : [-] Negative
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-11-Verify-Empty-Password-Shows-Validation.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that empty password shows Required validation.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-11-Verify-Empty-Password-Shows-Validation.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 2 — NEGATIVE: an empty password must be blocked by front-end validation.
test.describe('Login — Authentication', () => {
  ...
  // [-] Negative: submitting with an empty password should be blocked client-side.
  test('TC-LOGIN-11 — Empty password shows Required validation', async ({ page }) => {
    ...
    console.log('[STEP 3] Leave password empty...');
    checks.push({ label: 'Password left empty', ok: true, detail: 'empty' });
    ...
    printTcSummary('TC-LOGIN-11', 'Empty password shows Required validation', checks);

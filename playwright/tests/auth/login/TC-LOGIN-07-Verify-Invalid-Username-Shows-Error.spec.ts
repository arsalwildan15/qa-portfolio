/**
 * ========================================================================
 * TC-LOGIN-07 — Invalid username is rejected
 * Module  : Authentication — Login
 * Type    : [-] Negative
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-07-Verify-Invalid-Username-Shows-Error.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that invalid username is rejected.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-07-Verify-Invalid-Username-Shows-Error.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 2 — NEGATIVE: a username that does not exist must be rejected.
test.describe('Login — Authentication', () => {
  ...
  // [-] Negative: a username that doesn't exist should be rejected.
  test('TC-LOGIN-07 — Invalid username is rejected', async ({ page }) => {
    ...
    console.log(`[STEP 2] Fill username "InvalidUser"...`);
    await loginPage.fillUsername('InvalidUser');
    checks.push({ label: 'Fill username "InvalidUser"', ok: true, detail: 'filled' });
    ...
    const response = await loginPage.submitAndInterceptExpectingRejection('InvalidUser', VALID_PASSWORD);
    ...
    printTcSummary('TC-LOGIN-07', 'Invalid username is rejected', checks);

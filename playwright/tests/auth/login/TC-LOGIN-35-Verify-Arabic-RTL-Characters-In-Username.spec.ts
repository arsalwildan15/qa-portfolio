/**
 * ========================================================================
 * TC-LOGIN-35 — Arabic (RTL, non-Latin) username does not crash the server
 * Module  : Authentication — Login
 * Type    : [!] Edge
 * Target  : https://opensource-demo.orangehrmlive.com/web/index.php/auth/login
 * File    : TC-LOGIN-35-Verify-Arabic-RTL-Characters-In-Username.spec.ts
 * Path    : tests/auth/login/
 * ========================================================================
 *
 * DESCRIPTION:
 *   Verifies that arabic (RTL, non-Latin) username does not crash the server.
 *
 * API VERIFIED:
 ...
 *
 * RUN THIS TEST:
 *   npx playwright test tests/auth/login/TC-LOGIN-35-Verify-Arabic-RTL-Characters-In-Username.spec.ts --project=login-tests --headed
 *
 * RUN ALL LOGIN TESTS:
...
// GROUP 5 — EDGE: non-Latin script (Arabic/RTL) must be sent as valid UTF-8.
test.describe('Login — Authentication', () => {
  ...
  // [!] Edge: non-Latin script (Arabic) in the username must be sent as valid UTF-8, not break the request.
  test('TC-LOGIN-35 — Arabic (RTL, non-Latin) username does not crash the server', async ({ page }) => {
    ...
    console.log('[STEP 2] Fill Arabic username "مستخدم"...');
    await loginPage.fillUsername('مستخدم');
    checks.push({ label: 'Fill Arabic username', ok: true, detail: 'filled' });
    ...
    checks.push({ label: 'POST /auth/validate sent (UTF-8)', ok: submitted, detail: submitted ? 'sent' : 'NOT SENT' });
    ...
    printTcSummary('TC-LOGIN-35', 'Arabic (RTL, non-Latin) username does not crash the server', checks);
